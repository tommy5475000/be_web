...
export const getInvoiceById = async (accessToken, id) => {
  try {
    const response = await axios.get(`https://public.kiotapi.com/invoices/${id}`, {
      headers: {
...
export const getInvoiceByCode = async (accessToken, code) => {
  try {
    const response = await axios.get(`https://public.kiotapi.com/invoices/code/${code}`, {
      headers: {
        Authorization: accessToken,
        Retailer: "benthanhtsc",
      },
...
const updateOneBill = async ({ id = null, code = null }) => {
  ...
  if (bill) {
    ...
    console.log(`⚠️ Không tìm thấy bill với ${id ? "id " + id : "code " + code}`);
  }
};
